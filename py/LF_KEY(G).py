...
def load_data():
    ...
    try:
        ...
        messagebox.showerror("Error", f"เกิดปัญหาตอนโหลดข้อมูล: {e}")
    finally:
        if conn:
            conn.close()
...
btn_del = tk.Button(btn_frame, text="🗑️ ลบสินค้าที่เลือก", command=delete_product, bg="#e74c3c", fg="white", font=btn_font, width=20, relief="flat", activebackground="#c0392b", activeforeground="white")
...
tree.pack(pady=10, padx=25, fill="both", expand=True)
